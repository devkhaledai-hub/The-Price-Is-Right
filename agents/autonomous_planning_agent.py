...
class AutonomousPlanningAgent(Agent):
    ...
    def plan(self, memory: List[str] = []) -> Optional[Opportunity]:
        """
        Deterministic orchestration version (no LLM tool calling)
        """
        ...
        # 1️⃣ Scan for deals
        results_json = self.scan_the_internet_for_bargains()

        if results_json == "No deals found":
        ...
        # 2️⃣ Estimate value for each deal
        for deal in results.get("deals", []):
            description = deal.get("product_description") or deal.get("description")
            deal_price = deal.get("price")
            url = deal.get("url")
            estimate = self.ensemble.price(description)
            discount = estimate - deal_price

            self.log(f"Deal: {description}")
            self.log(f"Price: {deal_price}, Estimated: {estimate}, Discount: {discount}")
        ...
        # 3️⃣ Notify best deal
        description = best_deal.get("product_description") or best_deal.get("description")


        if best_deal:
            self.notify_user_of_deal(
                description,
                best_deal["price"],
                best_estimate,
                best_deal["url"]
